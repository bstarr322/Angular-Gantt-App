...
gantt._formatLink = function(link){
	...
	for(var i = 0; i < from.length; i++){
		...
		for(var j = 0; j < to.length; j++){
			...
			var lag = fromTask.lag + toTask.lag;

			var subtaskLink = gantt.copy(link);
			subtaskLink.source = fromTask.task;
			subtaskLink.target = toTask.task;
			subtaskLink.lag = (subtaskLink.lag || 0) + lag;
...
gantt._autoSchedulingGraph = {
	...
	topologicalSort: function(edges){
		...
		var hash = vertices.reduce(function(summ, value) {
			summ[value] = {id: value, $source:[], $target:[], $incoming: 0};
			return summ;
		}, {});
...
gantt._autoSchedulingPlanner = {
	...
	applyProjectPlan: function(projectPlan){
		...
		for(var i = 0; i <  projectPlan.length; i++){
			...
			if (gantt.callEvent("onBeforeTaskAutoSchedule", [task, newDate, link, predecessor]) === false) {
				break;
			}
